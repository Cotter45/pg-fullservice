...
exports.generateJavascriptServer = exports.generateTypescriptServer = void 0;
...
function generateTypescriptClass(schema, path) {
    ...
    ${schema.columns
        ...
        if (column.isNullable) {
            validator += `Joi.${(0, util_1.convertDBTypeToTsType)(column.type).toLowerCase()}().allow(null).allow('')`;
        }
        else {
            validator += `Joi.${(0, util_1.convertDBTypeToTsType)(column.type).toLowerCase()}().required()`;
        }
        return validator;
    })
        .join(',\n    ')}
    ...
    classContent += `            text: 'INSERT INTO ${schema.tableName}(${schema.columns
        .sort((a, b) => {
        if (a.name < b.name) {
            return -1;
        }
    ...
    classContent += `            values: [${schema.columns
        ...
        .map((column) => `data.${column.name} || ${(0, util_1.typeToValue)(column.type)}`)
        .join(', ')}]\n`;
    classContent += `        };\n`;
    classContent += `        try {\n`;
    ...
    classContent += `        const client = this.connectDB();\n`;
    classContent += `        const query = {\n`;
    classContent += `            text: 'UPDATE ${schema.tableName} SET ${schema.columns
        .sort((a, b) => {
        if (a.name < b.name) {
            return -1;
        }
...
const generateTypescriptServer = (schemaFile, path) => {
...
exports.generateTypescriptServer = generateTypescriptServer;
...
const generateJavascriptServer = (schemaFile, path) => {
    const keys = Object.keys(schemaFile);
    for (const key of keys) {
        const table = schemaFile[key];
        const columns = table.map((column) => ({
...
exports.generateJavascriptServer = generateJavascriptServer;
